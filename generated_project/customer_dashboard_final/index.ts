...
// Safe schema generation helper with production-grade fallback control
function safeZodToJsonSchema(zodSchema: any, { env = process.env.NODE_ENV } = {}): any | null {
  try {
    return zodToJsonSchema(zodSchema, { $refStrategy: 'none' });
  } catch (error) {
    console.error('Schema generation failed:', error);
    
    // 本番環境では非公開（安全側）、開発では制限付きフォールバック
    if (env === 'production') {
      return null; // ツール除外
    }
    ...
    return { 
      type: 'object', 
      additionalProperties: false, // 緩すぎを防ぐ
      description: 'DEV: Schema generation failed - limited fallback'
    };
  }
}
...
// Track schema generation failures for server status
let hasSchemaFailures = false;

const server = new Server(SERVER_INFO, { capabilities: CAPABILITIES });

// tools/list with output schema and production-grade error handling
server.setRequestHandler(ListToolsRequestSchema, async () => {
  const tools = Object.entries(handlers).map(([name, h]) => {
    try {
      const inputSchema = safeZodToJsonSchema(h.Input, { env: process.env.NODE_ENV });
      const outputSchema = safeZodToJsonSchema(h.Output, { env: process.env.NODE_ENV });
      
      // 本番環境でスキーマ生成失敗時はツールを除外
      if (!inputSchema || !outputSchema) {
        hasSchemaFailures = true; // ★ 必須フラグ設定
        console.warn(`Tool ${name} excluded due to schema generation failure`);
        return null;
      }
      ...
      hasSchemaFailures = true; // ★ エラー時もフラグ設定
      return null;
    }
  }).filter(Boolean) as any[];
  
  // DEGRADED状態を通知（tools数減少で推測可能）
  if (hasSchemaFailures) {
    console.warn(`🚨 MCP Server running in DEGRADED mode - some tools excluded due to schema failures`);
  }
