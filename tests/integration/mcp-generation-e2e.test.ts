  ...
  operations: [
    {
      ...
      inputs: [
        { name: 'name', type: 'string', required: true, validation: {} },
    ...
    },
    {
      ...
      inputs: [
        { name: 'limit', type: 'number', required: false, validation: {} }
      ],
      outputs: {
        type: 'object',
        schema: {
 ...
 * 生成されたMCPサーバーの機能をテスト
 */
async function testMcpServerFunctionality(toolDir: string): Promise<void> {
 ...
 * tools/list リクエストをテスト
 */
async function testToolsList(serverProcess: any): Promise<void> {
  return new Promise((resolve, reject) => {
    const request = {
      jsonrpc: '2.0',
      id: 'test-1',
...
test('E2E - Generated tool with UI components', async () => {
  ...
  try {
    ...
    assert(await readFile(join(integrationPath, 'ui-tool-broker.ts'), 'utf-8'), 
           'UI tool broker should be generated');
