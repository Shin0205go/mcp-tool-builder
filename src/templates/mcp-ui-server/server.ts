...
// UI tool imports
import { uiCustomerDashboard } from './tools/ui.customer.dashboard.js';
  ...
  private setupToolHandlers(): void {
    ...
    this.server.setRequestHandler(ListToolsRequestSchema, async () => {
      return {
        tools: [
          {
            name: "ui.customer.dashboard",
            description: "Customer management dashboard UI component with search and CRUD capabilities",
            inputSchema: {
              type: "object",
              properties: {
                limit: {
                  type: "number",
                  description: "Maximum number of customers to load initially",
                  default: 20
                },
                preset: {
                  type: "string", 
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      ...
      try {
        switch (name) {
          case "ui.customer.dashboard":
            return await uiCustomerDashboard(args || {});
