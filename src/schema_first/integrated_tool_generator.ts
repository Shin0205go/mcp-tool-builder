...
export class IntegratedToolGenerator {
   ...
   * 型定義ファイルを生成
   */
  private async generateTypeDefinitions(outputDir: string): Promise<void> {
    const typeCode = `/**
...
export interface McpTool<TInput extends z.ZodType, TOutput extends z.ZodType> {
  name: string;
  description: string;
  inputSchema: TInput;
  outputSchema: TOutput;
  run(input: z.infer<TInput>, ctx: { userId?: string; idempotencyKey?: string }): Promise<z.infer<TOutput>>;
}
...
`;
    ...
    const filename = join(outputDir, 'types', 'mcp.ts');
    await fs.writeFile(filename, typeCode);
  }
...
import { ${entityName}DAO } from '../dao/${entityName}DAO.js';
import { 
...
/**
 * ${this.escapeString(operation.description) || operationName}
 */
...
> = {
  name: '${operationName}',
  description: '${this.escapeString(operation.description || `${operationName} for ${entityName}`)}',
  ...
  async run(input: z.infer<typeof ${this.getInputSchemaName(operationName, entityName)}>, ctx: { userId?: string; idempotencyKey?: string }) {
...
/**
 * ${this.escapeString(operation.description) || operationName} - Business Logic
 */
...
${tool.operations.map(op => `- **${op.name}**: ${this.escapeString(op.description) || 'No description'}`).join('\n')}
...
`;
  ...
  /**
   * Escape single quotes in strings for use in JavaScript/TypeScript string literals
   */
